...
if($r>0){
    ...
    $pdf->Cell(90,5,'Recibo N#: '.$aux['idrecibo'],0,1,'c');
	$pdf->SetFont('Arial','B',12);
	...
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(65,15,' ',0,0,'L',0);
	$pdf->Cell(65,5,'RECIBO DE PAGO',0,0,'C');
	$pdf->Ln();
	...
	if($r2>0){
		while($row=$resultado2->fetch_assoc()){
			...
			/*$k=1;
				...
				$pdf->Cell(0,18,'Detalle de factura',0,1,'c');
				$pdf->SetFont('Arial','B',12);
				$pdf->Cell(25,15,' ',0,0,'L',0);
	...
	$pdf->Ln();
	$pdf->SetFont('Arial','B',16);
	$pdf->Cell(80,5,'Total Pagado '.number_format($aux['ingreso'],2,',','.').' Bs.',0,1,'c');
	$pdf->Ln();
	$pdf->Cell(90,5,'Por Pagar: '.number_format($aux['restante'],2,',','.').' Bs.',0,1,'c'); 
	$pdf->Ln();
